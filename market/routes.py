    ...
    if request.method == 'POST':
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                flash(f'Purchased {p_item_object.name} for £{p_item_object.price}.', category='success')
            else:
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        flash(f'Account succesfully created. You are now logged in as {user_to_create.username}', category='success')
        return redirect(url_for('market_page'))
